...
__cdecl void fault_handler(idt::regs32_t regs)
{
    ...
    System::Kernel::Video.Clear((uint32_t)VBE_COLOR::red);
    printf("PlutoOS Kernel Panic\n\n\n");
    printf("Exception: %s",exceptions[regs.identifier.trap_number]);
    System::Kernel::Video.Render();
    asm volatile("cli");
    asm volatile("hlt");
    for(;;);
}
